...
def solverlinear(matrA,vecb):
    vectb=vecb.copy()
    matrA=coo_matrix(matrA)
    matrA=matrA.tocsc()
    matrA=matrA.astype(np.float64)
...
def get_normcontrol(vecphi0_hat,vecphi1_hat,k,h1,h2,M,N1,N2,C1,C2):
    matphi_hat=Solver_Wave(M,N1,N2,k,C1,C2,np.zeros((M+2,N2)),np.zeros((M+2,N1)),np.zeros((M+2,N2)),np.zeros((M+2,N1)),vecphi0_hat[1:,1:],vecphi1_hat[1:,1:])
    ...
    normalphi_2=(-1./h2)*matphi_hat[:,:,0]
    normalphi_4=(-1./h2)*matphi_hat[:,:,-1]
    int1=integ_phi_psi(k,h2,M,N2,normalphi_1,normalphi_1)+integ_phi_psi(k,h2,M,N2,normalphi_3,normalphi_3)
    int2=integ_phi_psi(k,h1,M,N1,normalphi_2,normalphi_2)+integ_phi_psi(k,h1,M,N1,normalphi_4,normalphi_4)
    normL2=np.sqrt(int1+int2)
    print(normL2)
